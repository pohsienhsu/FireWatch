...
### INSTALLATION
1. cd CODE/
2. Unzip installer.zip. This will create directory installer/.
3. Download datasets:
...
4. cd datasets/. Run the dataset pre-processing scripts:

	To process fires data, run the following commands (in exactly this order):
		1. python3 ../scripts/clean_fire_data.py
		2. python3 ../scripts/convert_fires_sqlite_to_csv.py
	...
	To process AQI data, run the following command:
		python3 ../scripts/aqi_col_rename.py 
	
	This will reformat the AQI annual data to that which is required by our choropleth map.
